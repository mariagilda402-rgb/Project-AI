...
async function loadVideos() {
    ...
    container.innerHTML = data.length ? '' : '<div style="text-align:center; color:var(--text-secondary); margin-top:20px;">Nenhum v√≠deo salvo offline.</div>';
    data.forEach(v => {
...
async function loadHabits() {
    ...
    container.innerHTML = data.length ? '' : '<div style="text-align:center; color:var(--text-secondary); margin-top:20px;">Nenhum h√°bito cadastrado.</div>';
    data.forEach(habit => {
        const el = document.createElement('div');
        el.className = 'list-item glass';
        ...
        el.innerHTML = `
            <div class="item-main">
                ...
                <span class="item-subtitle">Streak: üî• ${habit.current_streak}</span>
            </div>
...
window.toggleHabit = async (id, btn) => {
    ...
    if (btn.classList.contains('done')) {
        sendLocalNotification('H√°bito Conclu√≠do!', 'Voc√™ ganhou pontos de experi√™ncia!');
...
async function loadFinances() {
    ...
    container.innerHTML = data.length ? '' : '<div style="text-align:center; color:var(--text-secondary); margin-top:20px;">Sem transa√ß√µes.</div>';
    data.slice(0, 15).forEach(t => {
        const el = document.createElement('div');
        el.className = 'list-item glass';
        el.innerHTML = `<div class="item-main"><span class="item-title">${t.description || 'Transa√ß√£o'}</span><span class="item-subtitle" style="color:${t.type==='income'?'#00b894':'#fd79a8'}">${t.type==='income'?'+':'-'} $${t.amount}</span></div>`;
        container.appendChild(el);
    });
}
...
window.startPomodoro = () => {
    ...
    pomoInterval = setInterval(() => {
        if (pomoTimeLeft > 0) {
            ...
            sendLocalNotification('Foco ConcluÌdo!', 'VocÍ ganhou +50 XP por 25 minutos de estudo.');
