...
class TTSService:
    ...
    def __init__(
        ...
        if self.provider == 'edge' and not self.edge_tts_available:
            print('[TTS] Edge TTS selecionado, mas o pacote edge_tts não está instalado. Fallback configurado automaticamente.')
    ...
    def _effective_provider_order(self) -> list[str]:
        order = self.provider_order or resolve_tts_provider_order(self.provider)
        if not getattr(self, 'edge_tts_available', True):
            order = [p for p in order if p != 'edge']
        return order

    def _speak_one_chunk(self, text: str) -> None:
        if not text.strip():
            return
        order = self._effective_provider_order()
        if self.provider == 'edge' and not getattr(self, 'edge_tts_available', True):
            print('[TTS] Edge TTS não está instalado; pulando provedor edge e usando fallback.')
    ...
    def _check_edge_tts_availability(self) -> bool:
        try:
