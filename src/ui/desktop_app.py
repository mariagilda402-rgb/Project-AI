...
class DesktopApi:
    ...
    def _clear_task_queue(self) -> None:
        """Descarta todas as tarefas pendentes na fila."""
        import queue
    ...
    def get_config(self):
        ...
        return {
            ...
            'edge_tts_available': getattr(self._tts, 'edge_tts_available', False),
    ...
    def install_edge_tts(self):
        try:
            ...
            command = [sys.executable, "-m", "pip", "install", "edge_tts"]
            result = subprocess.run(command, capture_output=True, text=True, timeout=600)
            if result.returncode != 0:
                stderr = (result.stderr or result.stdout or "").strip()
                error_message = stderr.splitlines()[-1] if stderr else "Falha ao instalar edge_tts."
                return {"ok": False, "error": error_message}

            if self._tts and hasattr(self._tts, "_check_edge_tts_availability"):
                try:
                    self._tts.edge_tts_available = self._tts._check_edge_tts_availability()
                except Exception:
                    self._tts.edge_tts_available = True

            return {"ok": True, "message": "edge_tts instalado com sucesso."}
        except Exception as e:
            return {"ok": False, "error": str(e)}
    ...
    def nexus_get_dashboard_data(self):
        try:
            from src.services.nexus_service import get_nexus_service
            return get_nexus_service().get_dashboard_summary()
        except Exception as e:
            print(f"[DesktopApi] nexus_get_dashboard_data: {e}")
            return None
    ...
    def generate_persona(self, character_name):
        """Usa a LLM (instância existente) para gerar um prompt de personalidade para um personagem."""
        if not character_name or not character_name.strip():
            return None
        if not self._llm:
            print("[DesktopApi] LLM não disponível para gerar persona.")
            return None
        try:
            system = (
    ...
    def close_module(self):
        try:
            app = APP_INSTANCE
            if app and app.panel_window:
                app.panel_window.hide()
            if app and app.ghost_window:
                app.ghost_window.show()
                app.ghost_window.evaluate_js("document.body.style.visibility = 'visible';")
            return True
        except Exception as e:
            print(f"[DesktopApi] close_module erro: {e}")
            return False
    ...
    def minimize_module(self):
        try:
            app = APP_INSTANCE
            if app and app.panel_window:
                app.panel_window.minimize()
            return True
        except Exception as e:
            print(f"[DesktopApi] minimize_module erro: {e}")
            return False
    ...
    def maximize_module(self):
        try:
            ...
            print(f"[DesktopApi] maximize_module erro: {e}")
            return False
    ...
    def show_activity_hud(self, config_json: str = "{}") -> bool:
        """Exibe o HUD de atividade animado com os itens fornecidos."""
        app = APP_INSTANCE
        if not app:
            return False
        try:
            config = json.loads(config_json or "{}")
            return app.show_activity_hud(config)
        except Exception as e:
            print(f"[DesktopApi] show_activity_hud: {e}")
            return False

    def hide_activity_hud(self) -> bool:
        """Oculta o HUD de atividade imediatamente."""
        app = APP_INSTANCE
        if not app:
            return False
        try:
            return app.hide_activity_hud()
        except Exception as e:
            print(f"[DesktopApi] hide_activity_hud: {e}")
            return False
...
class DesktopApp:
    ...
    def show_activity_hud(self, config: dict) -> bool:
        """Exibe o HUD de atividade animado sobre qualquer janela."""
        if not self._ensure_hud_window():
            return False
        try:
            try:
                ...
                pass
            safe = json.dumps(config, ensure_ascii=False).replace('\\', '\\\\').replace("'", "\\'")
            self._hud_window.evaluate_js(f"hudShow({safe});")
            return True
        except Exception as e:
            print(f"[DesktopApp] show_activity_hud: {e}")
            return False

    def hide_activity_hud(self) -> bool:
        """Oculta o HUD de atividade imediatamente."""
        if not self._hud_window:
            return False
        try:
            self._hud_window.evaluate_js("hideHud();")
            try:
            ...
            print(f"[DesktopApp] hide_activity_hud: {e}")
            return False

    def _ensure_hud_window(self) -> bool:
        if self._hud_window:
            return True
        hud_path = (Path(__file__).resolve().parent / "nexus_hud.html").as_uri()
        try:
            ...
            print(f"[DesktopApp] _ensure_hud_window: {e}")
            self._hud_window = None
            return False

    def notify_hud_hidden(self) -> bool:
        """Callback do JS quando o HUD terminou a animação de ocultar."""
        if not self._hud_window:
            return False
        try:
            self._hud_window.hide()
            ...
            print(f"[DesktopApp] notify_hud_hidden: {e}")
            return False
    ...
    def start(self):
        ...
        # ── HUD de Atividade (janela always-on-top, click-through) ──
        hud_path = (Path(__file__).resolve().parent / "nexus_hud.html").as_uri()
        self._hud_window = webview.create_window(
            "__nexus_hud__",
            url=hud_path,
